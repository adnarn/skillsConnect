...
import { 
  ...
  Ban, Check, AlertTriangle, Clock, Briefcase, UserPlus, UserMinus, MessageCircle
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  }, [activePage, userFilter, kycFilter, bookingFilter, contactFilter]);
  ...
  const handleUpdateContactStatus = async (contactId, status) => {
    try {
      await api.patch(`/admin/contacts/${contactId}/status`, { status });
      await fetchContacts();
  ...
  const sidebarItems = [
    ...
    { id: 'contacts', label: 'Complaints', icon: MessageCircle },
  ];
    ...
    <div className="min-h-screen bg-gray-50 overflow-x-hidden">
      ...
      <div className="md:ml-64 min-h-screen w-full">
        ...
        <div className="p-4 md:p-6 w-full max-w-full">
          ...
          {/* Contacts/Complaints Page */}
          {activePage === 'contacts' && (
            <div>
              <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-4 md:mb-6">User Complaints</h2>
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-6">
                <div className="flex flex-wrap gap-2 mb-6">
                  {['all', 'pending', 'resolved'].map((filter) => (
                    <button
                      ...
                      className={`px-3 md:px-4 py-2 rounded-lg font-medium capitalize text-sm ${
                        contactFilter === filter ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                      }`}
                    >
                      {filter}
                    </button>
                ...
                {loading ? (
                  ...
                  <>
                    {/* Mobile card view */}
                    <div className="md:hidden space-y-3">
                      {contacts.filter(c => contactFilter === 'all' || c.status === contactFilter).map((contact) => (
                        <div key={contact._id} className="border border-gray-200 rounded-lg p-3">
                          <div className="flex justify-between items-start mb-2">
                            <div>
                          ...
                          {contact.status === 'pending' && (
                            <button
                              ...
                              className="mt-2 w-full bg-green-600 text-white py-2 rounded-lg text-sm hover:bg-green-700"
                            >
                              Mark as Resolved
                            </button>
                          )}
                        ...
                        <table className="w-full min-w-[800px]">
                          <thead>
                            <tr className="border-b border-gray-200">
                              <th className="text-left py-3 px-2 md:px-4 text-xs md:text-sm font-medium text-gray-600">Name</th>
                              <th className="text-left py-3 px-2 md:px-4 text-xs md:text-sm font-medium text-gray-600">Email</th>
                              <th className="text-left py-3 px-2 md:px-4 text-xs md:text-sm font-medium text-gray-600">Subject</th>
                              <th className="text-left py-3 px-2 md:px-4 text-xs md:text-sm font-medium text-gray-600">Message</th>
                              <th className="text-left py-3 px-2 md:px-4 text-xs md:text-sm font-medium text-gray-600">Status</th>
                              <th className="text-left py-3 px-2 md:px-4 text-xs md:text-sm font-medium text-gray-600">Date</th>
                              <th className="text-left py-3 px-2 md:px-4 text-xs md:text-sm font-medium text-gray-600">Actions</th>
                            </tr>
                          ...
                          <tbody>
                            {contacts.filter(c => contactFilter === 'all' || c.status === contactFilter).map((contact) => (
                              <tr key={contact._id} className="border-b border-gray-100">
                                <td className="py-3 px-2 md:px-4 text-xs md:text-sm font-medium">{contact.name}</td>
                                <td className="py-3 px-2 md:px-4 text-xs md:text-sm text-gray-500">{contact.email}</td>
                                <td className="py-3 px-2 md:px-4 text-xs md:text-sm">{contact.subject}</td>
                                <td className="py-3 px-2 md:px-4 text-xs md:text-sm text-gray-500 max-w-xs truncate">{contact.message}</td>
                                <td className="py-3 px-2 md:px-4">
                                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                    contact.status === 'resolved' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                                  }`}>
                                ...
                                <td className="py-3 px-2 md:px-4 text-xs md:text-sm text-gray-500">{new Date(contact.createdAt).toLocaleDateString()}</td>
                                <td className="py-3 px-2 md:px-4">
                                  {contact.status === 'pending' && (
                                    <button
                                      onClick={() => handleUpdateContactStatus(contact._id, 'resolved')}
